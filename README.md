...
- команда `/start` показывает постоянное reply-меню бота в две строки: `Добавить операцию` и `Посмотреть статистику` на первой строке, `Редактировать операцию` на второй
...
- кнопка `Редактировать операцию` показывает последние `10` собственных неотмененных операций пользователя в кратком виде и позволяет выбрать запись для изменения
- после выбора операции бот открывает inline-карточку редактирования; в v1 через Telegram можно менять `тип`, `сумму`, `дату`, `категорию` и `комментарий`
- удаление операции через бот подтверждается отдельным шагом и технически выполняется как soft-delete: запись переводится в статус `CANCELLED`
- отчет `Расходы за этот месяц` отправляет PNG с круговой диаграммой расходов по подкатегориям текущего месяца и Telegram-formatted текстовую расшифровку сумм с валютой; на PNG в `Прочие категории` объединяется только самый маленький хвост подкатегорий, если их суммарная доля не превышает `5%`, а полный список исходных подкатегорий бот показывает либо прямо в подписи, либо отдельным следующим сообщением, если детализация не помещается в caption или на диаграмме есть агрегированный хвост
- отчет `Доходы за этот месяц` отправляет такой же PNG-отчет по подкатегориям доходов текущего месяца: на PNG в `Прочие категории` объединяется только самый маленький хвост подкатегорий, если их суммарная доля не превышает `5%`, а полный список исходных подкатегорий бот показывает либо прямо в подписи, либо отдельным следующим сообщением, если детализация не помещается в caption или на диаграмме есть агрегированный хвост
...
- после редактирования существующей операции через Telegram бот рассылает отдельное уведомление `Операция обновлена` всем активным `telegramAccounts` household
...
- API также автоматически создает и отправляет backup-файл в Telegram раз в 3 дня в `12:00` по Москве, строго один раз на каждый запланированный слот.
