...
- команда `/start` показывает постоянное reply-меню бота с кнопкой `Посмотреть статистику`
- кнопка `Посмотреть статистику` открывает inline-подменю отчетов; сейчас доступен отчет `Расходы за этот месяц`
- отчет `Расходы за этот месяц` отправляет PNG с круговой диаграммой расходов по категориям текущего месяца и текстовую расшифровку сумм; категории с долей меньше `5%` объединяются в `Прочие категории`
