...
- `@napi-rs/canvas` для генерации PNG-отчетов Telegram
...
- если база уже существовала до ввода Prisma-истории миграций, API-контейнер автоматически помечает baseline `20260407110000_init` как примененный и повторяет deploy
...
- отчет `Расходы за этот месяц` отправляет PNG с круговой диаграммой расходов по категориям текущего месяца и Telegram-formatted текстовую расшифровку сумм с валютой; на PNG категории с долей меньше `5%` объединяются в `Прочие категории`, а в тексте показывается полный список исходных категорий
- отчет `Доходы за этот месяц` отправляет такой же PNG-отчет по категориям доходов текущего месяца: на PNG мелкие категории с долей меньше `5%` объединяются в `Прочие категории`, а в текстовой расшифровке показывается полный список исходных категорий
